...
class Multicoco(nn.Module):
    ...
    def __init__(
        ...
        self.use_all_hidden_states = use_all_hidden_states
        self.last_n_layer = last_n_layer

        # tested with GPT2 and Llama3
        if isinstance(self.base_causallm, GPT2LMHeadModel):
            self.num_layers = len(self.base_causallm.transformer.h)
            self.embedding = self.base_causallm.transformer.get_input_embeddings()
        else:
            self.num_layers = len(self.base_causallm.model.layers)
            self.embedding = self.base_causallm.get_input_embeddings()

        if use_all_hidden_states:
            hidden_dim = self.embedding.embedding_dim
            self.layerIntegrator = MultiLayerIntegrator(
                self.num_layers, hidden_dim, integration_type
            )
    ...
    def forward(self, input_ids, attention_mask, labels, position_ids, **kwargs):
        ...
        for pass_idx in range(max_n_latents):
            ...
            # logits level majority vote
            selected_hidden_states = outputs.hidden_states[-self.last_n_layer:]
            
            logits_list = []
            # Calculate logits for each layer
            for h in selected_hidden_states:
                linear_output = self.base_causallm.lm_head(h) 
                logits_list.append(torch.softmax(linear_output, dim=-1))
